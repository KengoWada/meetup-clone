...
.PHONY: test
test:
	@SERVER_ENVIRONMENT=test go test -v -count=1 ./...

.PHONY: test-cov
test-cov:
	@SERVER_ENVIRONMENT=test go test -v -count=1 -coverprofile=coverage.out/cov.out ./... && go tool cover -html=coverage.out/cov.out -o=coverage.out/index.html
